...
class ItemNode(object):
    ...
    def __init__(self, frame, gpID, nodeNum):
        ...
        # caching requests for everything
        requests_cache.install_cache('nodeCache', backend='sqLite')
...
class EphemeralNode(ItemNode):
    def __init__(self, frame, gpID, nodeNum):
        ...
        # REQUESTS FOR GATHERING ITEM DATA
        gatherDataCacheEph = requests.get(f'https://xivapi.com/GatheringPoint/{self._GatheringPointID}?private_key=45484ebc973f443fae7a525fc0557ac832dacf8e079c47e195717cf0f6df32dd')

        # JSON CREATION ***********************

        # JSONS WITH GATHERING ITEM DATA

        gatheringPointData = gatherDataCacheEph.text
        parse_gatheringPointData = json.loads(gatheringPointData)
        ...
        locationMapName = parse_gatheringPointData['TerritoryType']['PlaceName']['Name']
        locationPlaceName = parse_gatheringPointData['PlaceName']['Name']
        self.SpawnTime0 = int(parse_gatheringPointData['GatheringPointTransient']['EphemeralStartTime'])
        fNodeSpawnTime0 = f'{self.SpawnTime0:04d}'
        ...
        # labels for spawn currentTime
        tk.Label(self._nodeFrame,
                 text=fNodeSpawnTime0 + " Eorzean Time", bg='#858585',
                 font=self._labelFont).grid(
            row=3, column=0,
            columnspan=2,
            sticky=tk.W)
    ...
    def IsActive(self, currentTime):
        if self.SpawnTime0 < currentTime < (self.SpawnTime0 + 360):
            self.active = True
            self._realWorldSeconds = self.FindRealSeconds(
                self.FindRemainingActiveTime(EorzeanClock.ConvertEorzea(self)))
        else:
    ...
    def FindNextSpawn(self, currentTime):
        spawn0 = self.SpawnTime0 - 40
        if spawn0 < currentTime:
            result = abs(spawn0 - currentTime)
        elif 2400 > currentTime > spawn0:
            result = abs(2360 - currentTime)
            result = spawn0 + result
        elif currentTime < spawn0:
            result = abs(spawn0 - currentTime)

        return result

    def FindRemainingActiveTime(self, currentTime):
        result = abs((self.SpawnTime0 + 360) - currentTime)
        return result
